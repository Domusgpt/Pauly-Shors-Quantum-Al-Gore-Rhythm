...
|--------|-------|---------|
| Attack reports (JSON) | `results/ibm/attack_*bit_ibm_fez_*.json` | Full metadata: timestamps, gate counts, keys, verification |
| Gate-level (JSON) | `results/ibm/gates_*bit_ibm_fez_*.json` | Pre-transpilation gate decomposition |
| OpenQASM 2.0 | `results/circuit_*.qasm` | Gate-level circuits (simulator) |
| Simulator reports | `results/attack_*bit.json` | Baseline simulator results for comparison |
